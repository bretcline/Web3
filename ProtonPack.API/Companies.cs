...
namespace ProtonPack.API
{
    public class Companies
    {
        ...
        [FunctionName("CompanyGetSummary")]
        [OpenApiOperation(operationId: "Company/GetSummary", tags: new[] { "name" })]
        [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
        ...
        public async Task<IActionResult> GetSummary(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "Company/GetSummary")] HttpRequest req)
        {
            var rc = await Utilities.ValidateSession(req, async (CompanyUser companyUser, dynamic data) =>
            {
                using var manager = new BusinessLogic.CompanyManager(companyUser);
        ...
        [FunctionName("CompanyGetContractSettings")]
        [OpenApiOperation(operationId: "Company/GetContractSettings", tags: new[] { "name" })]
        [OpenApiSecurity("function_key", SecuritySchemeType.ApiKey, Name = "code", In = OpenApiSecurityLocationType.Query)]
        [OpenApiParameter(name: "name", In = ParameterLocation.Query, Required = true, Type = typeof(Customer), Description = "The **Name** parameter")]
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "application/json", bodyType: typeof(Customer), Description = "The OK response")]
        public async Task<IActionResult> GetContractSettings(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "Company/GetContractSettings")] HttpRequest req)
        {
            var rc = await Utilities.ValidateSession(req, async (CompanyUser companyUser, dynamic data) =>
            {
                using var manager = new BusinessLogic.SystemSettingsManager(companyUser);
                Guid ContractTypeID = WebThree.Shared.Utilities.GetGuid(data.ContractTypeID);
                return await manager.GetContractSettings(ContractTypeID);
            }, (CompanyUser companyUser, Exception err) => Utilities.LogError(_logger, companyUser, err, req), false).ConfigureAwait(false);
            return rc;
        }
    }
